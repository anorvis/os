...
function runMigrations(db: Database): void {
  ...
  if (current < 3) {
    db.exec("INSERT INTO schema_migrations (version, applied_at) VALUES (3, datetime('now'))");
  }
  if (current < 4) {
    db.exec(`
...
function ensureTaskSessionsColumns(db: Database): void {
  ...
  addColumn(db, "task_sessions", "start_at", "TEXT NOT NULL DEFAULT '1970-01-01T00:00:00.000Z'");
  addColumn(db, "task_sessions", "end_at", "TEXT NOT NULL DEFAULT '1970-01-01T00:00:00.000Z'");
...
function seedIntegrationCatalog(db: Database): void {
  const integrations = [
    ["google", "Google Workspace", "life", "Calendar, Gmail, and Drive context for scheduling and retrieval.", JSON.stringify(["calendar.read", "calendar.sync"]), "oauth2"],
...
function seedProviderDefinitions(db: Database): void {
  ...
  const providers = [
    ["google", "Google Workspace", "life", JSON.stringify(["calendar.read", "calendar.sync"]), "oauth2"],
    ["google-calendar", "Google Calendar", "life", JSON.stringify(["calendar.read", "calendar.sync"]), "oauth2"],
    ["google-tasks", "Google Tasks", "productivity", JSON.stringify(["tasks.read", "tasks.sync"]), "oauth2"],
    ["spotify", "Spotify", "life", JSON.stringify(["music.read"]), "oauth2"],
    ["hevy", "Hevy", "health", JSON.stringify(["workouts.sync"]), "token"],
  ];
  const insert = db.query("INSERT OR IGNORE INTO provider_definitions (id, display_name, category, capabilities_json, auth_type, enabled, created_at, updated_at) VALUES (?1, ?2, ?3, ?4, ?5, 1, ?6, ?6)");
