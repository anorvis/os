...
export function upsertTaskSession(id: string, input: { start: string; end: string }, now = new Date()): TaskSession | null {
  const startAt = normalizeDate(input.start, "start");
  const endAt = normalizeDate(input.end, "end");
  if (new Date(endAt).getTime() <= new Date(startAt).getTime()) throw new Error("end must be after start");
...
function normalizeTaskPatch(patch: TaskPatch): TaskPatch {
  ...
  if (normalized.date) normalized.dueAt = new Date(`${normalized.date}T23:59:00`).toISOString();
...
function taskDate(input: Record<string, unknown>): string | null | undefined {
  if (typeof input.date === "string") return new Date(`${input.date}T23:59:00`).toISOString();
  return nullableString(input.dueAt);
}
