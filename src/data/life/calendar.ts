...
export function parseCalendarEventInput(value: unknown): CalendarEventInput | null {
  ...
  const startAt = typeof value.startAt === "string" ? value.startAt : value.startDateTime;
  const endAt = typeof value.endAt === "string" ? value.endAt : value.endDateTime;
  if (typeof value.summary !== "string" || typeof startAt !== "string" || typeof endAt !== "string") return null;
  try {
...
export function parseCalendarEventPatch(value: unknown): CalendarEventPatch | null {
  if (!isRecord(value)) return null;
  ...
  if ("startAt" in value || "startDateTime" in value) {
    const startAt = "startAt" in value ? value.startAt : value.startDateTime;
    if (startAt !== undefined && typeof startAt !== "string") return null;
    patch.startAt = startAt;
  }
  if ("endAt" in value || "endDateTime" in value) {
    const endAt = "endAt" in value ? value.endAt : value.endDateTime;
    if (endAt !== undefined && typeof endAt !== "string") return null;
    patch.endAt = endAt;
  }
