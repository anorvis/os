...
export const settings = action({
  ...
  handler: async (ctx, args): Promise<{
    ...
    const connection = (await ctx.runQuery(internal.integrations.connection, {
...
export const start = action({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.runMutation(internal.integrations.beginGoogle, {
      workspaceId: args.workspaceId,
...
export const completeOAuth = internalAction({
  ...
  handler: async (ctx, args): Promise<string> => {
    const oauthState = await ctx.runMutation(internal.integrations.consumeGoogleState, {
      stateHash: stateHash(args.state),
      now: Date.now(),
    });
    const connection = await ctx.runQuery(internal.integrations.connectionByWorkspace, {
    ...
    await ctx.runMutation(internal.integrations.finishGoogle, {
      workspaceId: oauthState.workspaceId,
...
async function syncPage(
  ...
  nextPageToken?: string;
    ...
    const connection = await ctx.runQuery(
      internal.integrations.connectionByWorkspace,
    ...
    for (let offset = 0; offset < events.length; offset += 100) {
      const result = await ctx.runMutation(internal.integrations.upsertGoogleEvents, {
        workspaceId,
        system: true,
