...
describe("Finance capabilities", () => {
  it("aggregates canonical balances with exact fixed-point arithmetic", async () => {
    ...
    await client.mutation(api.finance.saveAccount, {
      name: "Wallet A",
      type: "cash",
      currency: "BTC",
      balance: "0.10000000",
    ...
    const dashboard = await client.query(api.finance.dashboard, {});
  ...
  it("converts every monetary record into the requested reporting currency", async () => {
    ...
    const dashboard = (await client.action(api.financeDashboard.dashboard, {
      currency: "CAD",
    })) as {
  ...
  it("carries sparse account history forward and subtracts liabilities", async () => {
    ...
    const assetId = await client.mutation(api.finance.saveAccount, {
      name: "Savings",
      type: "savings",
      currency: "CAD",
    });
    const loanId = await client.mutation(api.finance.saveAccount, {
      name: "Loan",
      type: "loan",
      currency: "CAD",
    });
  ...
  it("upserts transactions by a stable dedupe key", async () => {
    const { client } = await owner();
    const accountId = await client.mutation(api.finance.saveAccount, {
      name: "Checking",
      type: "checking",
      currency: "CAD",
    });
    const firstId = await client.mutation(api.finance.saveTransaction, {
      accountId,
    ...
    const secondId = await client.mutation(api.finance.saveTransaction, {
      accountId,
      dedupeKey: "statement:42",
    ...
    const rows = await client.query(api.finance.listTransactions, { accountId });
  ...
  it("imports CSV rows in resumable idempotent batches", async () => {
    ...
    const result = await client.action(api.financeImport.importCsv, args);
    expect(result).toMatchObject({ applied: 2, skipped: 0 });
    const repeated = await client.action(api.financeImport.importCsv, args);
  ...
  it("keeps the same instrument distinct across brokerage accounts", async () => {
    const { t, client, workspaceId } = await owner();
    const firstAccountId = await client.mutation(api.finance.saveAccount, {
      name: "Brokerage A",
      type: "investment",
      currency: "USD",
    });
    const secondAccountId = await client.mutation(api.finance.saveAccount, {
      name: "Brokerage B",
      type: "investment",
      currency: "USD",
    });
    for (const accountId of [firstAccountId, secondAccountId]) {
      await t.mutation(internal.integrations.applySnapTradeAccountData, {
        workspaceId,
        system: true,
        accountId,
        balances: [],
  ...
  it("rejects monetary values outside signed int64 storage", async () => {
    const { client } = await owner();
    await expect(
      client.mutation(api.finance.saveAccount, {
        name: "Impossible",
        type: "cash",
        currency: "CAD",
