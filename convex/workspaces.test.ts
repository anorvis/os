...
describe("workspace authorization", () => {
  it("creates one owner workspace and default preferences", async () => {
    ...
    const firstId = await asOwner.mutation(api.workspaces.ensureDefault, {});
    const secondId = await asOwner.mutation(api.workspaces.ensureDefault, {});
    expect(secondId).toBe(firstId);

    const viewer = await asOwner.query(api.workspaces.viewer, {});
  ...
  it("rejects anonymous and cross-workspace reads", async () => {
    ...
    const workspaceId = await t
      ...
      .mutation(api.workspaces.ensureDefault, {});

    await expect(
      t
        .withIdentity({ subject: outsiderId })
        .query(api.workspaces.viewer, { workspaceId }),
