...
import {
  ...
  zeroDecimal,
  type Decimal,
} from "../../capability/finance/decimal";
...
export const dashboard = action({
  ...
  handler: async (ctx, args): Promise<unknown> => {
    ...
    const dashboard = (await ctx.runQuery(api.capability.finance.dashboard, {})) as Row;
    ...
    const total = accounts.reduce<Decimal>((sum, account) => {
      ...
      return addDecimal(sum, parseDecimal(decimalText(account.balance)));
    }, zeroDecimal());
...
function aggregateHistory(
  ...
  const accountTypes = new Map(
    accounts
      ...
      .flatMap((account) => {
        ...
        return id ? [[id, text(account.type) ?? "checking"] as const] : [];
      }),
  );
  const accountValues = values.filter((row) => {
    const id = text(row.accountId);
    return id !== null && accountTypes.has(id);
  });
  ...
  for (const date of [...rowsByDate.keys()].sort()) {
    for (const row of rowsByDate.get(date) ?? []) {
      ...
      if (accountId !== null) latest.set(accountId, row);
    }
...
function decimalText(value: unknown): string {
  ...
  throw rateError("Invalid decimal value in finance projection");
}
