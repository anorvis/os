...
describe("native Wiki", () => {
  it("keeps append-only revisions and rejects stale edits", async () => {
    ...
    const created = await client.mutation(api.wiki.save, {
      path: "Architecture/Backend",
      markdown: "# Backend\n\nConvex.",
    });
    await expect(
      client.mutation(api.wiki.save, {
        pageId: created.pageId,
        path: "Architecture/Backend.md",
        markdown: "# Backend\n\nStale edit.",
      }),
    ).rejects.toThrow("Wiki page changed since it was loaded");
    const updated = await client.mutation(api.wiki.save, {
      pageId: created.pageId,
    ...
    const history = await client.query(api.wiki.history, {
      pageId: created.pageId,
    });
  ...
  it("preserves rename aliases and rejects path collisions", async () => {
    ...
    const first = await client.mutation(api.wiki.save, {
      path: "Notes/First",
      aliases: ["First Note"],
      markdown: "# First",
    });
    await client.mutation(api.wiki.rename, {
      pageId: first.pageId,
      path: "Notes/Renamed",
    });
    const oldPath = await client.query(api.wiki.get, { path: "Notes/First" });
    const explicitAlias = await client.query(api.wiki.get, { path: "First Note" });
    expect(oldPath?._id).toBe(first.pageId);
    expect(explicitAlias?._id).toBe(first.pageId);
    const second = await client.mutation(api.wiki.save, {
      path: "Notes/Second",
      markdown: "# Second",
    });
    await expect(
      client.mutation(api.wiki.rename, {
  ...
  it("repairs unresolved graph edges when a target appears", async () => {
    vi.useFakeTimers();
    const { t, client } = await owner();
    const source = await client.mutation(api.wiki.save, {
      path: "Source",
      markdown: "See [[Future Target|the future]].",
    });
    expect(await client.query(api.wiki.unresolvedLinks, {})).toHaveLength(1);
    const target = await client.mutation(api.wiki.save, {
      path: "Future Target",
      markdown: "# Future Target",
    });
    await t.finishAllScheduledFunctions(vi.runAllTimers);
    expect(await client.query(api.wiki.unresolvedLinks, {})).toHaveLength(0);
    const backlinks = await client.query(api.wiki.backlinks, {
  ...
  it("validates attachment tenancy and reuses bytes without merging owners", async () => {
    ...
    const firstAttachment = await t.mutation(internal.wiki.storeAttachment, {
      ...input,
      pageId: first.pageId,
      storageId: firstStorageId,
    });
    const secondAttachment = await t.mutation(internal.wiki.storeAttachment, {
      ...input,
      pageId: second.pageId,
    ...
    const otherWorkspaceId = await other.mutation(api.workspaces.ensureDefault, {});
    await expect(
      t.mutation(internal.wiki.storeAttachment, {
        ...input,
  ...
  it("authorizes file delivery, supports ranges, and downloads active content", async () => {
    const { t, client, workspaceId } = await owner();
    const page = await client.mutation(api.wiki.save, {
      path: "Files/Delivery",
      markdown: "# Delivery",
    });
    ...
    const stored = await t.mutation(internal.wiki.storeAttachment, {
      workspaceId,
      pageId: page.pageId,
      storageId,
      name: "unsafe.html",
