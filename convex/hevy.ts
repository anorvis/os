...
export const saveSettings = action({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.runMutation(internal.integrations.saveHevyCredentials, {
      workspaceId: args.workspaceId,
  ...
  handler: async (ctx, args) => {
    ...
    for (let guard = 0; guard < 200; guard += 1) {
      const result: ProviderStepResult = await ctx.runAction(
        internal.hevy.syncScheduledStep,
        { workspaceId, cursor },
      );
  ...
  handler: async (ctx, args): Promise<ProviderStepResult> => {
    ...
    if (state.kind === "workouts") {
      ...
      const counts: { created: number; updated: number } =
        await ctx.runMutation(internal.integrations.upsertHevyWorkouts, {
          workspaceId: args.workspaceId,
          system: true,
          workouts,
        });
    ...
    const counts: { created: number; updated: number } =
      await ctx.runMutation(internal.integrations.upsertHevyMeasurements, {
        workspaceId: args.workspaceId,
        system: true,
        measurements,
      });
