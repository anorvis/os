...
import { searchOpenFoodFacts, searchTheMealDb } from "./foodProviders";
...
export const searchFood = action({
  ...
  handler: async (ctx, args): Promise<{
    ...
    if (provider === "all" || provider === "recipe") {
      const recipes = await ctx.runQuery(api.recipes.search, {
        workspaceId,
        query,
      });
      results.push(
...
export const searchRecipes = action({
  ...
  handler: async (ctx, args) => {
    await ctx.runQuery(internal.integrations.authorizeWorkspace, {});
    const query = args.query.trim();
    return {
      query,
...
export const importRecipe = action({
  ...
  handler: async (ctx, args): Promise<unknown> => {
    ...
    const id = await ctx.runMutation(api.recipes.save, {
      workspaceId,
      title: input.title,
      source: input.source,
    ...
    const recipe = await ctx.runQuery(api.recipes.get, { workspaceId, id });
    if (!recipe) {
