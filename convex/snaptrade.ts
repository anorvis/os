...
export const saveSettings = action({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.runMutation(internal.integrations.saveSnapTradeCredentials, {
      workspaceId: args.workspaceId,
...
async function applyBatches(
  ...
  for (let offset = 0; offset < data.balances.length; offset += 100) {
    const result = await ctx.runMutation(
      internal.integrations.applySnapTradeAccountData,
      {
        workspaceId,
        system: true,
        accountId,
...
async function syncScheduledPage(
  ...
  if (state === undefined) {
    ...
    const ids: Array<{
      ...
      internal.integrations.upsertSnapTradeAccounts,
      {
        workspaceId,
        system: true,
        accounts,
  ...
  const accounts: Array<{
    ...
    accountId: Id<"financeAccounts">;
    currency: string;
  }> = await ctx.runQuery(internal.integrations.snapTradeAccounts, {
    workspaceId,
  });
