...
export const importCsv = action({
  ...
  handler: async (
    ...
    if (args.transactions !== undefined) {
      ...
      const started = await ctx.runMutation(internal.capability.finance.import.begin, {
      ...
      try {
        ...
        for (let offset = started.cursor; offset < rows.length; offset += 50) {
          const result = await ctx.runMutation(internal.capability.finance.import.applyBatch, {
            jobId: started.jobId,
        ...
        await ctx.runMutation(internal.capability.finance.import.finish, {
          jobId: started.jobId,
        ...
        await ctx.runMutation(internal.capability.finance.import.fail, {
          jobId: started.jobId,
