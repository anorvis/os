...
async function resolveLinks(
  ...
  await ctx.scheduler.runAfter(0, internal.capability.wiki.resolveLinksToPage, {
    workspaceId,
    pageId,
    paths: [...new Set(paths)],
    pathIndex: 0,
...
async function appendRevision(
  ...
  await ctx.scheduler.runAfter(0, internal.capability.wiki.indexRevision, {
    pageId: input.page._id,
    revisionId,
  });
  return revisionId;
...
export const indexSource = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (offset < text.length && ordinal < 500) {
      await ctx.scheduler.runAfter(0, internal.capability.wiki.indexSource, {
        sourceId: source._id,
        offset,
        ordinal,
      });
