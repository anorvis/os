...
async function owner() {
  ...
  await client.mutation(api.platform.workspace.ensureDefault, {});
  return { t, client };
}
...
describe("Health capabilities", () => {
  it("returns meals and active profile in the day dashboard", async () => {
    ...
    await client.mutation(api.capability.health.saveMeal, {
      name: "Breakfast",
      mealType: "meal",
      loggedAt: 1_500,
      calories: 500,
    ...
    await client.mutation(api.capability.health.saveMacroProfile, {
      targetCalories: 2_400,
      proteinGrams: 180,
    });

    const dashboard = await client.query(api.capability.health.dashboard, {
      dayStart: 1_000,
      dayEnd: 2_000,
    });
  ...
  it("replaces nested workout children without leaving orphans", async () => {
    ...
    const workoutId = await client.mutation(api.capability.health.saveWorkout, {
      title: "Strength",
      startedAt: 1_000,
    ...
    await client.mutation(api.capability.health.saveWorkout, {
      id: workoutId,
    ...
    const workout = await client.query(api.capability.health.getWorkout, { id: workoutId });
  ...
  it("upserts imported recipes by source identifier and keeps provenance", async () => {
    ...
    const firstId = await client.mutation(api.capability.health.recipes.save, input);
    const secondId = await client.mutation(api.capability.health.recipes.save, {
      ...input,
      title: "Updated soup",
    ...
    const recipe = await client.query(api.capability.health.recipes.get, { id: firstId });
  ...
  it("searches saved food through the authenticated Convex action", async () => {
    const { client } = await owner();
    await client.mutation(api.capability.health.recipes.save, {
      title: "Protein oats",
      calories: 420,
      proteinGrams: 32,
      carbsGrams: 50,
    ...
    const result = await client.action(api.capability.health.search.searchFood, {
      query: "protein",
      provider: "recipe",
    });
  ...
  it("keeps exactly one active macro profile", async () => {
    const { t, client } = await owner();
    await client.mutation(api.capability.health.saveMacroProfile, { targetCalories: 2_000 });
    const activeId = await client.mutation(api.capability.health.saveMacroProfile, {
