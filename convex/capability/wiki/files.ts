  ...
  handler: async (ctx, args): Promise<Id<"wikiSources">> => {
    ...
    const text =
      ...
      (mimeType.startsWith("text/") ||
        ...
        ? await blob.text()
        : undefined;
    const stored = await ctx.runMutation(internal.capability.wiki.storeSource, {
      workspaceId,
...
export const registerAttachment = action({
  ...
  handler: async (ctx, args): Promise<Id<"wikiAttachments">> => {
    ...
    const stored = await ctx.runMutation(internal.capability.wiki.storeAttachment, {
      workspaceId,
      storageId: args.storageId,
      pageId: args.pageId,
...
export const semanticSearch = action({
  ...
  handler: async (
    ...
    const rows: Array<Doc<"wikiChunks">> = await ctx.runQuery(
      internal.capability.wiki.chunksById,
      {
        workspaceId,
