...
type Payload = FunctionArgs<typeof api.platform.migration.legacyImport.applyBatch>;
...
describe("legacy SQLite import", () => {
  test("is idempotent and preserves task/finance/wiki relationships", async () => {
    ...
    const first = await client.mutation(api.platform.migration.legacyImport.applyBatch, payload);
    const second = await client.mutation(api.platform.migration.legacyImport.applyBatch, payload);
    vi.useFakeTimers();
    ...
    await t.mutation(internal.capability.wiki.indexRevision, { pageId: stored.pages[0]._id, revisionId: stored.pages[0].currentRevisionId });
  ...
  test("patches multiple accounts that point at a global import job", async () => {
    ...
    await client.mutation(api.platform.migration.legacyImport.applyBatch, payload);
  ...
  test("upserts provider credentials without exposing plaintext", async () => {
    const { t, client } = await owner();
    const payload = emptyPayload();
    const credentials = { algorithm: "aes-256-gcm" as const, keyVersion: 1, nonce: "nonce", ciphertext: "ciphertext" };
  ...
  test("imports raw wiki material as sources, not searchable pages", async () => {
    ...
    await t.mutation(internal.capability.wiki.indexSource, { sourceId: stored.sources[0]._id });
